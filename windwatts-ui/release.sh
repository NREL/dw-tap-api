...
export APPFLEET_BUILD_ARGS='{"main": {"image_detail": "./appfleet-config/mainDetail.json", "dockerfile": "./Dockerfile", "ecr_repo": "$(REGISTRY-IDS).dkr.ecr.us-west-2.amazonaws.com/nrel-windwatts-ui-${APPFLEET_RELEASE_NAME}", "target_arg": ""}}'
...
make -f "$MAKEFILE_PATH" V=1 build push --provenance=false
