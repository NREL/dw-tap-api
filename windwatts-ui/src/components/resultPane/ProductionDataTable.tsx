...
const ProductionDisplay = ({ 
  ...
  const windSpeeds = sortedData.map(({ values }) => Number(values['Average wind speed (m/s)']));
  ...
  const productions = sortedData.map(({ values }) => Number(values['kWh produced']));
      ...
      <Table size="small" sx={{ width: '100%' }}>
        ...
        <TableBody>
          {sortedData.map(({ time, values }) => {
            ...
            const windSpeedPercentage = windSpeedRange > 0 
